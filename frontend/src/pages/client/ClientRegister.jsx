...
import { C, FieldInput, ZelligeBg } from '../../components/UI'
...
export default function ClientRegister() {
  ...
  return (
    <div style={{
      ...
      <div style={{
        width: '100%', maxWidth: 420, background: C.bg, borderRadius: 32,
        ...
        <div style={{ padding: '0 28px 32px' }}>
          ...
          <form onSubmit={submit}>
            <FieldInput label="Nom ou Pseudo" value={name} onChange={e => setName(e.target.value)} placeholder="Amira M." />
            ...
            <div onClick={() => setNewsletter(!newsletter)} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', marginBottom: 24, cursor: 'pointer' }}>
              <div style={{ width: 20, height: 20, borderRadius: 6, border: `2px solid ${newsletter ? C.tealL : C.border}`, background: newsletter ? `linear-gradient(135deg,${C.teal},${C.tealL})` : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, color: C.bg, flexShrink: 0, marginTop: 1, transition: 'all 0.2s' }}>
                {newsletter ? '✓' : ''}
              </div>
              <span style={{ fontSize: 12, color: C.muted, lineHeight: 1.6 }}>Je souhaite recevoir les nouveautés et offres exclusives des boutiques SOUK.</span>
            </div>
          ...
          <div style={{ textAlign: 'center', marginTop: 24, fontSize: 12, color: C.muted }}>
            ...
            <Link to={`/client/login?returnUrl=${encodeURIComponent(returnUrl)}`} style={{ color: C.tealL, textDecoration: 'none', fontWeight: 600 }}>Se connecter</Link>
          </div>
        </div>
      </div>
    </div>

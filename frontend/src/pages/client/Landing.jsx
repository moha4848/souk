    ...
    <div style={{ background: C.bg, minHeight: '100vh', color: C.text, fontFamily: "'Outfit', sans-serif" }}>
      ...
      <header className="navbar" style={{ 
        ...
        <div onClick={() => navigate('/')} style={{ cursor:'pointer', display:'flex', alignItems:'center', gap:15 }}>
           <div style={{ position:'relative', width:48, height:48, background:C.emerald, borderRadius:16, display:'flex', alignItems:'center', justifyContent:'center', boxShadow:`0 0 20px ${C.emerald}40` }}>
              <span style={{ color:'#fff', fontWeight:900, fontSize:24 }}>S</span>
           </div>
           <div style={{ display:'flex', flexDirection:'column' }}>
              <span style={{ fontSize: 24, fontWeight: 900, letterSpacing: 2, fontFamily: "'Outfit', sans-serif", lineHeight:1 }}>SOUK</span>
              <span style={{ fontSize: 9, letterSpacing: 3, color: C.emerald, fontWeight:800 }}>EMERALD EDITION</span>
           </div>
        </div>
        
        <div style={{ display: 'flex', gap: 50, alignItems: 'center' }}>
          <div className="nav-links" style={{ display:'flex', gap:35, fontSize:12, fontWeight:800, letterSpacing:1.5 }}>
             <span onClick={() => navigate('/explore')} style={{ cursor:'pointer', transition:'0.3s' }} onMouseOver={e=>e.target.style.color=C.emerald} onMouseOut={e=>e.target.style.color=C.text}>EXPLORER</span>
             <span onClick={() => navigate('/register')} style={{ cursor:'pointer', color:C.emerald }}>VENDRE</span>
          </div>
          <div 
        ...
        <div className="hero-grid" style={{ maxWidth: 1400, width: '100%', display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: 80, alignItems: 'center', position: 'relative', zIndex: 10 }}>
          ...
          <div className="hero-copy" style={{ animation: 'slideUp 1.2s cubic-bezier(0.23, 1, 0.32, 1)' }}>
            <div style={{ display:'flex', alignItems:'center', gap:18, marginBottom:35 }}>
               <Pill color={C.gold} bg={`${C.gold}15`}>NOUVELLE GÉNÉRATION</Pill>
            ...
            <p style={{ fontSize: 22, lineHeight: 1.7, color: C.muted, maxWidth: 600, marginBottom: 60, fontWeight:300 }}>
              Découvrez la plateforme SaaS qui propulse le génie marocain dans l'ère digitale. Une expérience de luxe, de confiance et d'innovation.
            </p>
              ...
              perspective: 1200
               ...
               <div style={{ height: '100%', background: C.bg, padding: 25, display:'flex', flexDirection:'column', gap:20 }}>
                  ...
                  <div style={{ flex:1, position:'relative', borderRadius:28, overflow:'hidden', border:`1px solid ${C.emerald}20`, display:'flex', alignItems:'center', justifyContent:'center' }}>
                     ...
                     <div style={{ fontSize:60, fontWeight:900, color:C.emerald, animation: 'glowemerald 3s infinite' }}>S</div>
                  </div>
      ...
      <section style={{ maxWidth: 1400, margin: '0 auto', padding: '120px 40px' }}>
        <div style={{ textAlign: 'center', marginBottom: 100 }}>
          <Ornament />
          <h2 style={{ fontFamily: "'Outfit', sans-serif", fontSize: 56, color: '#fff', fontWeight:800, letterSpacing:'-1px' }}>Collection Curatée</h2>
          <p style={{ color: C.muted, fontSize: 20, maxWidth:650, margin:'25px auto', fontWeight:300 }}>L'excellence marocaine sélectionnée par nos experts pour votre plaisir.</p>
        </div>

        {loading ? <Spinner /> : (
          <div className="trending-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 50 }}>
            {products.map((prod, i) => (
              <Card key={prod.id} hover onClick={() => navigate(`/store/${prod.vendor?.store_slug}/product/${prod.id}`)} style={{ padding: 0 }}>
                <div style={{ height: 420, background: C.surface2, position: 'relative', overflow:'hidden' }}>
                  {prod.image_url ? (
                  ...
                  <div style={{ 
                    position: 'absolute', bottom: 30, right: 30, background: 'rgba(255,255,255,0.95)', 
                    color: '#000', padding: '12px 25px', 
                    borderRadius: 100, fontWeight: 900, fontSize: 18, boxShadow:'0 15px 30px rgba(0,0,0,0.3)'
                  }}>
                    {prod.price} <span style={{ fontSize: 11, opacity:0.6 }}>MAD</span>
                  </div>
                </div>
                
                <div style={{ padding: 35, display: 'flex', flexDirection: 'column', gap: 20 }}>
                  <h3 style={{ fontSize: 28, margin: 0, color: '#fff', fontWeight:800 }}>{prod.name}</h3>
                  <div style={{ display:'flex', gap:12 }}>
                     <Pill color={C.emerald}>PREMIUM</Pill>
                     <Pill color={C.gold} bg={`${C.gold}15`}>MAROC</Pill>
                  </div>
                  <GoldBtn style={{ marginTop: 10, width:'100%' }}>DÉCOUVRIR</GoldBtn>
                </div>
              </Card>
            ))}
          </div>
      ...
      <footer style={{ background: '#050505', padding: '140px 80px', borderTop: `1px solid ${C.border}` }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', textAlign:'center' }}>
           <div style={{ fontSize:40, fontWeight:900, color:C.emerald, marginBottom:40 }}>SOUK</div>
           <p style={{ color: C.muted, maxWidth: 650, margin: '0 auto 60px', lineHeight: 1.8, fontSize:20, fontWeight:300 }}>La révolution digitale de l'artisanat marocain commence ici.</p>
